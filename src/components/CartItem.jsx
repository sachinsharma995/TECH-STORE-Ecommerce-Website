...
const Cartitem = ({ item }) => {
  ...
  return (
    <>
      <div
        className="flex flex-col items-start justify-between sm:flex-row p-4 sm:p-6 mb-4 bg-gray-900 rounded-xl shadow-2xl
    border-gray-800 transition duration-300 hover:border-orange-600/50 "
      >
        ...
        <div className="flex items-center justify-between sm:justify-end w-full sm:w-2/5 sm:mt-0 space-x-4">
          <div className="flex items-center border border-gray-700 rounded-full overflow-hidden shadow-lg ">
            <button
              onClick={decreaseQ}
              className="p-2 text-gray-400 bg-gray-800 hover:bg-gray-700 transition duration-150 w-8
           ...
           h-8 flex items-center justify-between"
            ...
            <button onClick={()=>removeFromCart(item.id , true)} className="p-3 bg-red-800/20 text-red-400 hover:bg-red-800/40
            transition duration-150 shadow-md rounded-full">
              <X className=""/>
            </button>
        </div>
      </div>
